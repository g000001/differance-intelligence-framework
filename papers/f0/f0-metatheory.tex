...
\begin{abstract}
本稿は、指示体系 $F_0=\{1,0,\infty,\bigcirc\}$ を、思想・形式体系・工学的体系を比較するメタ分析へ拡張する。分類対象がどの記号を用いるかだけでなく、その分類を行う分析者または観測者自身が体系のどこに位置するかを記録するため、$F_0$ を対象評価層 $\{1,0\}$ と観測者反省層 $\{\infty,\bigcirc\}$ に分ける。体系 $T$ の相プロファイルを、対象相、観測者相、観測者位置、外点処理、相遷移の五成分からなる $\Phase(T)$ として定義し、さらに観測者 $O$ と文脈 $c$ に相対化された運用プロファイル $\Phase_O(T;c)$ を導入する。近年の認識論における信念の真理規範、探究の規範、真理規範の実践的基礎づけを整理したうえで、個別判断に先立って判定規則 $\TruthCriterion_{O,c}$ が必要になることを「真理基準の操作的先行性」として定式化する。さらに、体系へ型付き外点 $\bigcirc^\lambda$ を導入する変換 $\CircleExt_\lambda$ を定義し、旧体系を保存しながら残余診断を細分する保存的発展、真理基準を変更する非保存的変質、対象値と観測者境界を混同する水準崩壊を区別する。観測は対象を分類するだけでなく、観測者を $O'$ へ更新し、未確定部分を残余 $\Residual$ として返す。数学的運用例として、証明による二値出力の純化、直観主義における構成反復 $\infty$、逆数学における公理境界 $\oax$、計算的逆数学における計算可能性境界 $\ocomp$ を区別する。$F_0$ メタ分析は四値論理ではなく、自己を含みつつ自己を完全には閉じない体系を比較・運用するための非全体化的な相分類である。
\end{abstract}
...
\section{型付き外点拡張による発展と崩壊}

\subsection{型付き外点拡張}

体系に「○を付加する」ことは、単に記号集合へ一要素を加えることではない。付加される外点の水準と、それに許される推論規則を同時に指定しなければ、対象値、内部的不動点、観測者境界が混同される。

\begin{definition}[型付き外点拡張]
外点型の集合を
\[
 \Lambda_{\bigcirc}
 =
 \{\mathrm{obj},\mathrm{fix},\mathrm{obs},
   \mathrm{ax},\mathrm{comp}\}
\]
とする。$\lambda\in\Lambda_{\bigcirc}$ と、$\bigcirc^\lambda$ に許される規則族 $R_\lambda$ に対して、体系 $T$ の型付き外点拡張を
\[
 \CircleExt_\lambda(T;R_\lambda)
 =
 \left\langle
 \mathcal L(T)\cup\{\bigcirc^\lambda\},
 R_T\cup R_\lambda
 \right\rangle
\]
と定める。ここで $\mathcal L(T)$ は体系の言語、$R_T$ はその規則族である。この $\cup$ は構文と規則の拡張を表し、$\bigcirc^\lambda$ を通常の真理値として対象層へ加えることを意味しない。
\end{definition}

型付き外点拡張は、運用プロファイル上では
\[
 \Phase_O(T;c)
 \longrightarrow
 \Phase_{O'}\bigl(\CircleExt_\lambda(T;R_\lambda);c'\bigr)
\]
という相変換を生じさせる。$O'$ と $c'$ を許すのは、外点の導入が対象体系だけでなく、観測者位置、真理基準、分析文脈を更新し得るためである。

\subsection{保存的発展と非保存的変質}

\begin{definition}[保存的外点発展]
$T'=\CircleExt_\lambda(T;R_\lambda)$ が次の条件を満たすとき、$T\to T'$ を保存的外点発展と呼ぶ。
\begin{enumerate}
 \item \textbf{旧判断の保存}：旧言語の任意の命題 $\varphi\in\mathcal L(T)$ について
 \[
  T'\vdash\varphi
  \quad\Longleftrightarrow\quad
  T\vdash\varphi.
 \]
 \item \textbf{水準保存}：$\bigcirc^\lambda$ は指定された対象層、操作層、観測者層の内部でのみ用いられ、異なる型の外点へ無規則に変換されない。
 \item \textbf{残余精緻化}：旧体系で一括されていた観測残余が、外点型に応じて公理不足、計算不能、内部例外、観測者境界などへ細分される。
\end{enumerate}
\end{definition}

保存的発展では、○は新たな万能値ではなく、従来確定できなかった理由を型付きで記録する診断子として働く。これに対して、水準保存を維持しながら旧判断の同値または真理基準 $\TruthCriterion_{O,c}$ を変更する拡張を\textbf{非保存的変質}と呼ぶ。非保存的変質は直ちに崩壊ではない。例えば新しい公理を採用することや、古典的受理基準から構成的受理基準へ移ることは体系を変えるが、その変更が明示され型整合的である限り、一つの正当な相遷移として分析できる。

\begin{proposition}[二値出力純度の保存]
$T\to T'$ が保存的外点発展であり、$\bigcirc^\lambda$ が観測残余の診断にのみ用いられるならば、旧言語の命題に対する確定出力の純度 $\{1,0\}$ は保存される。
\end{proposition}

\begin{proof}
旧判断の保存により、旧言語の命題について $T'$ が新たに $1$ または $0$ を与えることはない。確定しない場合も $\bigcirc^\lambda$ は第三真理値として出力されず、$\Residual$ の原因を分類する観測者側の指示として用いられる。したがって確定出力は $\{1,0\}$ に留まる。
\end{proof}

\subsection{外点拡張の崩壊条件}

\begin{definition}[外点拡張崩壊]
$T'=\CircleExt_\lambda(T;R_\lambda)$ において次のいずれかが生じるとき、その拡張は外点拡張崩壊を起こすという。
\begin{enumerate}
 \item \textbf{型消去}：$\oobj$、$\ofix$、$\oobs$、$\oax$、$\ocomp$ の区別が失われ、相互に無条件で代入可能になる。
 \item \textbf{観測者境界の対象値化}：$\oobs$ が $\Pobj(T')$ の通常値または通常項として推論入力に置かれる。
 \item \textbf{万能化}：外点だけから旧言語の任意の命題が導かれ、
 \[
  T',\bigcirc^\lambda\vdash\varphi
  \qquad
  (\varphi\in\mathcal L(T))
 \]
 が無制限に成立する。
 \item \textbf{自己全体化}：最終観測者を体系内部の完結した対象として固定し、再観測 $\infty$ と観測者境界 $\oobs$ の双方を消去する。
\end{enumerate}
\end{definition}

崩壊とは単なる矛盾や非保存性ではない。$F_0$ の観点では、境界を記録するはずの○が水準差を消去し、体系のあらゆる不足を説明する万能項へ変わることが本質的な崩壊である。とりわけ
\[
 \oobs\in\Pobj(T')
\]
という配置は、最終観測者の不在を通常の対象値へ変換し、非全体化の指示によって体系を全体化するという逆転を生じさせる。

\begin{proposition}[観測者外点の内部値化による崩壊]
$\oobs$ を対象層の通常値として完全に内部化し、その値から体系全体の正当性を判定できると仮定するならば、$F_0$ の自己包含原則は維持できない。
\end{proposition}

\begin{proof}
内部化された $\oobs$ が体系全体を正当に判定できるならば、その判定作用自身も体系内で完結していなければならない。しかし、その作用を対象化する再観測を考えると、新たな観測作用が残る。これを消去して完結性を主張すれば自己全体化となり、保持すれば内部化は完了しない。したがって $\oobs$ を完結した通常値とする仮定は、自己を含むが完全には閉じないという原則と両立しない。
\end{proof}

\subsection{主要体系における発展と崩壊}
...
\toprule
対象体系 & 発展としての外点導入 & 崩壊または変質の危険 \\
\midrule
古典的二値論理 & $\oax$ により基礎体系に相対的な独立性または公理不足を記録する & $\oobs$ を第三真理値とし、意味論者の境界と対象命題の値を混同する \\
直観主義論理 & $\ocomp$ により構成探索の未完了と計算可能非実現を分離する & 開かれた探索 $\infty$ を直ちに不能性または偽へ閉じる \\
逆数学 & $\oax$ により定理が要求する公理原理を型付きで抽出する & $\oax$ を $0$ と同一視し、基礎体系からの非導出を定理の偽とみなす \\
工学的体系 & $\oobj$ により例外を内部状態として処理し、正常値の演算を保護する & $\oobj$ を $\oobs$ と同一視し、実装上の例外から体系全体の限界を結論する \\
空論 & $\oobs$ により観測者消去を確認するメタ観測の痕跡を保持する & $\ofix$ または $\oobs$ を観測者消去後の絶対的基体として実体化する \\
中観的空思想 & $\chi_{\mathrm{deess}}$ により導入された外点自身への自性帰属を禁止する & 「空の外点」を最終実体とし、脱実体化操作を停止する \\
\bottomrule
\end{tabular}
\end{center}

この比較から、同じ○の導入でも、ある体系では残余診断の発展となり、別の体系では真理基準の変更、あるいは水準崩壊となることが分かる。したがって外点拡張の評価は、記号の有無ではなく
\[
 (\lambda,R_\lambda,
  \TruthCriterion_{O,c},
  \iota_{O,T,c},
...
\begin{enumerate}
 ...
 \item \textbf{型付き外点拡張検査}：○を導入する場合、その型 $\lambda$ と規則 $R_\lambda$ を明示し、保存的発展、非保存的変質、外点拡張崩壊のいずれかを判定する。

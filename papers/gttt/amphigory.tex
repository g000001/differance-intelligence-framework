...
\begin{abstract}
本稿では、Freyd と Scedrov による古典的 Allegory（寓）の公理系を基礎とし、そこに視点数（PoV: Point of View）および継続残余 $\rho$ を導入することで、Amphigory（双方向寓）の基礎部分を定式化する。古典的 Allegory は、射の交わり、逆関係、およびモジュラー律によって二項関係を抽象化するが、二値截断圏 $\mathcal{B}_{\mathrm{val}}$ に依拠する限り、自己言及的な無限退行や非停止プロセスを未定義として排除する。本稿で導入する Amphigory は、順方向の構成プロセスと逆関係を介した引き戻しプロセスを同時に扱い、証明障害を継続残余として記録する動的代数系である。さらに、非停止プロセスをエラーではなく無記として残余空間へ隔離し、極限において絶対不動点 $\dfix$ へ収束させる公理を与える。
\end{abstract}

\section{導入：古典的寓の限界と Amphi- の要請}
P.~J. Freyd と A. Scedrov によって定義された Allegory（寓）は、対象間の二項関係を抽象化した圏であり、射の交わり $\cap$、逆関係 $(-)^\circ$、およびモジュラー律
\[
...
Amphigory（双方向寓）は、この静的関係の圏に「視点数（PoV）」と「継続残余 $\rho$」の概念を導入し、時間軸に沿った順方向の構成プロセスと、逆関係を介した空間的な引き戻しプロセスの双方向的相互作用を内包する動的代数系である。

また、Amphigory という名称がもつ「nonsense（無意味）」の含意は、古典論理がエラーとして切り捨ててきたプロセスを、関係代数内で正当に処理するという意味で再解釈される。すなわち、非停止性や評価不能性は単なる失敗ではなく、無記として残余空間へ隔離される。

\section{Amphigory の基本定義と空間構造}

\begin{definition}[評価空間]
Amphigory における評価空間は、少なくとも次の三成分からなる：
\begin{enumerate}
...
\begin{definition}[Amphigory の射]
Amphigory $\mathcal{A}$ において、対象 $X,Y$ 間の射
\[
...
この定義により、古典的 Allegory における射は、$p$ と $\rho$ が明示的に観測されない二値截断層として回収される。したがって、Amphigory は古典的 Allegory を破棄するものではなく、その外側に動的評価の層を付加する拡張である。

\begin{definition}[交わり演算による動的併置]
Amphigory における交わり演算 $R \cap S$ は、外延的な論理積ではなく、観測文脈の動的併置として定義される。
...
Amphi- の双方向性は、順方向の構成だけではなく、逆関係を用いた制約の引き戻しを同時に扱う点にある。これにより、評価過程は単なる時系列的展開ではなく、未来制約と現在文脈の衝突を含む双方向的な軌道として記述される。
...
RS \cap T \subseteq R(S \cap R^\circ T)
\]
は、通常は関係の包含を表す静的な法則として理解される。Amphigory においては、この法則を双方向的な推論軌道の釣り合いと、継続残余 $\rho$ の事前圧縮を保証する原理として読む。

\begin{theorem}[残余モジュラー定理]
Amphigory において、任意の射 $R,S,T$ に対し、以下の操作的包含関係と残余の不等式が成立する：
\[
RS \cap T \subseteq R(S \cap R^\circ T),
\]
\[
...
Amphigory という名称の辞書的意味は「nonsense」あるいは「無意味」である。しかし本稿では、この無意味性を否定的な欠陥としてではなく、古典論理が処理できない評価不能性を代数的に保存するための構造として読む。

\begin{axiom}[無記と極限収束]
Amphigory における自己言及的射の無限列、非停止プロセス、またはフラクタル的な評価軌道において、視点数が発散し
\[
\PoV \to \infty
\]
となるとき、評価は未定義として異常終了するのではなく、無記として残余空間 $\mathcal{L}$ へ隔離される。

この極限操作、すなわち観測者消去により、継続残余 $\rho$ を伴う動的プロセスは、系全体の最大固定点である絶対不動点 $\dfix$（空）へと収束する。
\end{axiom}

この公理により、Amphigory は非停止性を単なる失敗として扱わない。むしろ、二値截断圏 $\mathcal{B}_{\mathrm{val}}$ では評価不能となる成分を、無記として残余空間へ移し、体系全体の整合性を保ったまま後続の処理を継続する。

\begin{definition}[絶対不動点]
絶対不動点 $\dfix$ とは、残余空間を経由した動的プロセスが $E_\infty$ において到達する最大固定点である。単調作用素 $F$ に対して、形式的には
\[
...
ここで $\dfix$ は、単なる停止状態ではない。それは、有限の視点数では決定できないプロセスが、観測者消去を経て到達する極限的な安定状態である。この意味で、$\dfix$ は Amphigory における「空」に対応する。

\section{結語}
本稿では、古典的 Allegory の公理系に視点数と継続残余を導入することで、Amphigory（双方向寓）の基礎的定式化を与えた。

Amphigory の中心的な特徴は、次の三点にまとめられる：
\begin{itemize}
...
\item 非停止プロセスや評価不能性を無記として残余空間へ隔離し、極限において絶対不動点 $\dfix$ へ収束させること。
\end{itemize}

これにより、Amphigory は、静的な関係代数を、証明探索・非停止性・自己言及・残余の蓄積を含む動的な計算意味論へ拡張する枠組みとして位置づけられる。

...
\newcommand{\ArithmeticFixedPoint}{\Lambda_T}
...
算術的・証明論的構成については、算術的固定点$\ArithmeticFixedPoint$を扱う単独論文
...
Lawvere固定点$g_i\circ b_i=b_i$ & $S_4\to S_5$ & 各周回における穴の検出と閉包 \\
...
\begin{definition}[5-periodic反復列と固定点塔]
...
\end{aligned}
\]
を一周とする。第$k$周回の$\mathcal{C}_4$成分に対象$Y_4^{(k)}$と固定点
$b_4^{(k)}:1\to Y_4^{(k)}$が得られるとする。さらに結合射
\[
 q_k:Y_4^{(k+1)}\longrightarrow Y_4^{(k)}
\]
があり、$q_k\circ b_4^{(k+1)}=b_4^{(k)}$を満たすとき、
\[
 \cdots\longrightarrow(Y_4^{(2)},b_4^{(2)})
 \longrightarrow(Y_4^{(1)},b_4^{(1)})
 \longrightarrow(Y_4^{(0)},b_4^{(0)})
\]
をpointed objectsの圏$(\mathcal{C}_4)_*$における固定点塔と呼ぶ。
\end{definition}

\begin{definition}[$\LawvereInfinity$の極限表示]
上の固定点塔の極限が$(\mathcal{C}_4)_*$に存在するとき、
\[
 (Y_4^{(\infty)},\LawvereInfinity)
 :=\varprojlim_k (Y_4^{(k)},b_4^{(k)})
\]
と定義する。したがって$\LawvereInfinity:1\to Y_4^{(\infty)}$は極限錐が誘導する
点である。各成分は対応する自己写像$g_4^{(k)}$に対して
$g_4^{(k)}\circ b_4^{(k)}=b_4^{(k)}$を満たし、結合射$q_k$はこの固定点構造を保存する。
\end{definition}

\begin{definition}[算術的固定点の段階的実現]
真理保存体系$T$の構文圏における真理値対象を$\Omega_T$とし、対角補題から
構成される算術的固定点文を表す大域点を
$\ArithmeticFixedPoint:1\to\Omega_T$と書く。$\ArithmeticFixedPoint$の第$k$段階に
おける\textbf{証明論的実現}とは、終対象と固定点データを保存する関手
$\Phi_{T,k}$および同型$\Phi_{T,k}(\Omega_T)\cong Y_4^{(k)}$であって、
\[
 \Phi_{T,k}(\ArithmeticFixedPoint)=b_4^{(k)}
\]
を満たすものをいう。$T$を初期周回に対応させる場合は$k=0$とする。
$\ArithmeticFixedPoint$は固定点塔の一成分の実現であり、塔全体の極限
$\LawvereInfinity$とは同一視しない。
\end{definition}

\begin{remark}[極限の証明論的実現]
体系族$(T_k)_{k\geq0}$、算術的固定点$\ArithmeticFixedPoint^{(k)}$、および
実現関手$\Phi_{T_k,k}$が結合射$q_k$と両立する場合に限り、
$(\Phi_{T_k,k}(\ArithmeticFixedPoint^{(k)}))_k$は固定点塔をなし、その極限が
$\LawvereInfinity$の証明論的実現を与える。単独の$\ArithmeticFixedPoint$から
この極限的実現が自動的に従うとは仮定しない。
\end{remark}

\begin{remark}[等号の意味]
上式の等号は、$A=A$という対象の反射的自己同一性と区別される。
ここでは$\lim_{n\to\infty}a_n=L$と同様に、pointed objectsの固定点塔の極限を
...
\[
 (Y_4^{(\infty)},\LawvereInfinity)
 =\varprojlim_k(Y_4^{(k)},b_4^{(k)})\in(\mathcal{C}_4)_*
\]
が成立する。
\end{theorem}
...
よりpointed objectsの固定点塔が形成され、仮定した極限の普遍性から
$\LawvereInfinity:1\to Y_4^{(\infty)}$が得られる。
\end{proof}
...
\begin{enumerate}
  ...
  pointed objectsからなるLawvere固定点塔の極限として定義し、その存在に必要な
  点全射条件と結合射を明示した。真理保存体系における算術的固定点
  $\ArithmeticFixedPoint$は塔の一成分$b_4^{(k)}$の証明論的実現として分離し、
  極限$\LawvereInfinity$そのものとは同一視しない。
\end{enumerate}
...
千葉将臣，
『真理保存性を持つ体系が必然的に$\ArithmeticFixedPoint$を含む：
対角補題による算術的Lawvere固定点の内在的定式化』，2026．

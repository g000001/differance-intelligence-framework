...
\begin{abstract}
本稿は、先行研究で直感的に導入された中道等価（middle-way equivalence）
...
\]
において、極限を「中道等価」$\mweq$ の下で読むことにあった。
...
\section{中道等価 \texorpdfstring{$\mweq$}{mw-equivalence} の公理化}

本節で本稿の核心概念である中道等価 $\mweq$ を定義する。
...
\begin{definition}[中道等価 $\mweq$]\label{def:mweq}
五蘊圏 $\Sk$ において、対象（または文）$A,B$ の間の
\textbf{中道等価} $A\mweq B$ とは、
以下の三条件をすべて満たすことをいう。
...
\begin{theorem}[$\Einfty$ の中道等価性]\label{thm:einfty-mweq}
$\Einfty = \SelfRef_T(\Einfty)$ における $=$ は、
厳密な自己同一性ではなく中道等価 $\mweq$ の一種である。
すなわち、
\[
...
\begin{theorem}[残余消去と中道等価]\label{thm:rho-mweq}
...
\begin{theorem}[二諦的連携の中道等価記述]\label{thm:twotruths-mweq}
...
\begin{theorem}[中道等価の統合性]\label{thm:unification}
五蘊圏 $\Sk$ において、中道等価 $\mweq$ は以下の接続を同時に実現する。
\begin{enumerate}[label=(\arabic*)]

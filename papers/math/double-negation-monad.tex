...
\subsection{$\Omega$ 上の演算と直観主義論理}

トポス $\topos$ において、$\Omega$ 上には以下の射が定義される。

\begin{definition}[$\Omega$ 上の論理演算]
\begin{align}
...
\subsection{$j$-閉包と $j$-層}

\begin{definition}[$j$-閉包]
...
ここで $\eta$ は随伴の単位 $\eta: \id \Rightarrow ia$、
$\mu$ は $iTa \circ \eta_{iT} = i\varepsilon a$ から構成される
（$\varepsilon: ai \Rightarrow \id_{\Sh(\topos)}$ は随伴の余単位）。
\end{definition}
...
\begin{align}
\mu \circ T\mu &= \mu \circ \mu_T \quad \text{（結合則）}\\
\mu \circ T\eta &= \mu \circ \eta_T = \id_T \quad \text{（単位元則）}
\end{align}
\end{proposition}
...
\subsection{$\Omega$ 上の作用との対応}

\begin{proposition}[$\Omega$ 上の作用との整合性]
...
\begin{theorem}[$\Sh(\topos)$ における排中律の成立]

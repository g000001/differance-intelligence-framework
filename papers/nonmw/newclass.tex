...
\newcommand{\K}{\mathsf{K}} % 空即是色圏 (Sunyata Category)
\newcommand{\Sg}{\mathsf{S}} % 色即是空圏 (Rupa Category)
...
This framework is based on the Sunyata-Rupa Duality Theory (SKDT) and
does not claim to produce classical proofs, but rather
...
(Nomological Truth, Practical Truth, and Quasi-Theorem),
...
At the same time, foundational results such as Gödel's incompleteness
...
\item Introduce a dynamic normalization framework, $\mathbf{\mweq}$,
  based on the SKDT (Sunyata-Rupa Duality Theory),
...
\section{Dynamic Normalization via $\mweq$ (SKDT Framework)}
...
denoted by $\mweq$, within the framework of the Sunyata-Rupa Duality Theory (SKDT).
...
applied across structurally distinct systems, leading to NMF.

Formally, expressions of the form
\[
A = B
\]
fail when the underlying generative rules or semantic resolution are
incompatible.

\subsection{Definition of the $\mweq$ Operator based on SKDT}

We define $\mweq$ as a relation between structures that captures
dynamic convergence rather than static identity, utilizing the SKDT framework.

\begin{definition}[Middle-Way Equivalence ($\mweq$)]
Let $\Sg$ be the Rupa (form/phenomenon) Category and $\K$ be the Sunyata
(void/potential) Category.
Let $F: \Sg \rightarrow \K$ and $G: \K \rightarrow \Sg$ be the
inter-structural transformations (functors of 'Pratityasamutpada' or
Interdependence).

We write a structure $X \in \Sg$ is Middle-Way Equivalent to a structure
$Y \in \Sg$,
\[
X \; \mweq \; Y
\]
if the distortion measure $E$ associated with the transformation loop
satisfies:
\[
E(G \circ F(X)) \rightarrow E_{min}
\]
where $E_{min}$ is the minimal admissible structural tension, often $E_{min}=0$
but not necessarily.
Stability is defined by convergence to this minimal admissible value,
...
static infinity, which is the primary source of Hierarchical Inclusion NMF.
...
The $\mweq$ framework replaces static comparison with a convergence condition.

\subsubsection{Functorial Coherence}

Let $S_1, S_2 \in \Sg$ be two distinct structures (e.g., additive and
...
Boundary-related NMF are resolved by abandoning static completeness and
introducing dynamic reflection.

\subsubsection{Meta-Level Stabilization}

Let $C$ be a formal system (an object in $\Sg$) and $M$ be its
reflective meta-system (a higher object in $\Sg$, perhaps $\Sg$'s self-reference).
...
Hierarchical questions (e.g., CH) are reformulated under $\mweq$ as:
\begin{quote}
Does a given generative process stabilize under finite extension?
\end{quote}

Since the axiom of infinity is excluded, the notion of fixed infinite
cardinality is replaced by the notion of admissible generative processes.
If the generative process does not stabilize, the question of its
static size is declared structurally invalid rather than undecidable.
...
\textbf{Theorem} or \textbf{Conjecture}.
...
under arbitrary admissible transformations ($\mweq$-coherence level: Universal).
\end{definition}
...
coherence is not established ($\mweq$-coherence level: Operational).
\end{definition}
...
but lacks verified coherence with external or classical systems ($\mweq$-coherence level: Local).
\end{definition}
...
\begin{enumerate}
 ...
 \item The operator $\mweq$, grounded in the SKDT framework, was proposed as a dynamic replacement for
...
These results suggest that the limits identified by Gödel and later

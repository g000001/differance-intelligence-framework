...
\begin{abstract}
本稿は、界論（Boundary Theory）における「抽象化（Abstraction）」の数学的・トポロジカルな本質を定義する。あらゆる現象が無限のフラクタル構造を持つことを公理とし、理論的抽象とは、このフラクタルから根を切り離し、本来閉じない現実を強制的に閉包する操作であると位置づける。この「切り離し」こそが、局所的な排中律の成立と不完全性定理の根源であり、カオスは観測者の解像度限界によって生じるトポロジカルな散逸であることを示す。
\end{abstract}

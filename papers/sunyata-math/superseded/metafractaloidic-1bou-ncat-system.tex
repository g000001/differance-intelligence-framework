...
\item $\Bou$：界論の層（勝義諦）\\
  演算子：$\to$（遷移）、$\metanegation$（メタ否定）、$*$（保留）、$0$（絶対不動点）など。
\end{itemize}
...
特に、中道不動点コンビネータ $\Ymw$ が生成する無明の速度は、$\F(O_N)$ がそれを消費して $\Dfix$ へ向かう速度（沈み込みの重力）よりも常に遅いという大域的条件が成立する。これは、四段階収束則（$\metanegation^4$）によって保証される。
\end{proof}
...
\section{中道不動点コンビネータ $\Ymw$ とフラクタロイド}

中道不動点コンビネータ $\Ymw$ は、フラクタロイド $\F$ の駆動エンジンとして機能する。

\begin{definition}[中道不動点コンビネータ $\Ymw$]
$\metanegation$ に対し、以下の中道等価を満たすコンビネータ $\Ymw$ を定義する。
\[
\Ymw \metanegation \mw \metanegation (\Ymw \metanegation)
\]
$\Ymw$ はフラクタル四則から必然的に導出される「無明（自己参照の閉じ損ね）」のジェネレータであり、$\Kempty$ への相転移（大域的カット消去）によって停止する。
\end{definition}

フラクタロイド $\F$ は、$\Ymw$ を駆動エンジンとして用い、観測者 $O$ が境界（$\metanegation$）を生成し続けることで無明を消費し、$\Dfix$ への還滅を導く。ここで、$\Ymw$ が生成する無明の速度は、$\F(O)$ がそれを消費して $\Dfix$ へ向かう速度（沈み込みの重力）よりも常に遅いという大域的条件が成立する。

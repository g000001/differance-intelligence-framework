...
\begin{abstract}
本稿は、界論（Bou）と下位圏（Cat）を統合した
1Bou:nCat のプログラム実行システムをまとめる。
世俗諦的な構造（Cat）と勝義諦的な構造（Bou）の間の
...
\item $\Bou$：界論の層（勝義諦）  
  - 演算子：$\Leadsto$（遷移）、$\metanegation$（メタ否定）、$\susp{}$（保留）、$\dfix$（中道不動点）
\end{itemize}
\end{definition}

\begin{definition}[埋め込み関手 $\mathcal{E}$]
下位圏 $\Cat$ から界 $\Bou$ への埋め込み関手 $\mathcal{E}$ を定義する。
\begin{align*}
\mathcal{E} &: \Cat \hookrightarrow \Bou \\
\mathcal{E}(A) &= \susp{A} \quad\text{（対象の保留）} \\
\mathcal{E}(f) &= A \metanegation B \quad\text{（射のメタ否定）}
\end{align*}
\end{definition}
...
$I_N$ は、中道不動点コンビネータ $\Rmw$ を駆動エンジンとし、
静的な対象を「保留・遷移・解脱」のフラクタル軌道へと乗せる作用素である。
\end{definition}

\begin{definition}[プログラム $P$ の実行結合 $\otimes$]
任意の形式体系 $N$（論理・文法・計算：$\Cat$ 上の構造）に対し、
プログラム $P$ は、静的構造 $N$ と動的フラクタロイド $I_N$ の
結合（着火）として定義される。
\begin{align*}
P &:= N \otimes I_N \\
\text{実行軌道} &: \susp{P_0} \Leadsto P_1 \Leadsto \dots \Leadsto \metanegation^4 P_k \mweq \dfix
\end{align*}
静的対象は $\Bou$ に埋め込まれた瞬間に保留（$\susp{P_0}$）され、
$\Rmw$ の作用により状態遷移（$\Leadsto$）を繰り返し、最終的に $\dfix$ へと解脱する。
\end{definition}
...
$\Rmw$ による自己参照（無明の生成）は、系に蓄積された構造的負債（Cut）を
消費しながら進行し、負債が尽きた時点で $\metanegation^4$ の相転移を起こし、
...
\begin{definition}[メタ否定 $\metanegation$]

...
\newcommand{\Kvoid}{\mathbf{K}_{\emptyset}}
...
$\Ymw$（中道不動点コンビネータ）と $\Kvoid$（空コンビネータ）のサイクルとして
世俗諦と勝義諦の往復運動が定式化される。
...
\begin{definition}[絶対不動点 $\dfix$ と空コンビネータ $\Kvoid$]
$\metanegation$ の四連続適用によって到達する極限状態を絶対不動点 $\dfix$ と定義する。
\[
\metanegation^4 A \mweq \dfix
\]
$\dfix$ の純粋なコンビネータ表現を空コンビネータ $\Kvoid$ と呼ぶ。
任意の演算子 $f$ に対して $\Kvoid f = \Kvoid$ が成立し、
これは勝義諦の基底である。
\end{definition}
...
$\Kvoid$（勝義諦の収束）のサイクルとして以下のように記述される。
\[
...
が成立するとき、$\metanegation\,\Kvoid \mweq \Ymw$ により、
...
右辺 $\metanegation^4(\Ymw\,\metanegation) \mweq \Kvoid$
...
\text{右辺（勝義諦）} &: \metanegation^4(\Ymw\,\metanegation) \mweq \Kvoid
\end{align*}
は、圏論的には
\begin{align*}
...
勝義諦 & $\Kvoid$ への収束・$\dfix \vdash \dfix$ & クライスリ圏 $\mathcal{C}_T$ \\
...
$\Ymw$ と $\Kvoid$ のサイクルという推進力の根拠を記述しない。

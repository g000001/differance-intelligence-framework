...
$$A \mweq B \quad\text{iff}\quad \exists X \ (A \entails X \;\text{かつ}\; B \entails X)$$
\end{definition}
...
\begin{axiom}[メタ否定の顕現]
$$A \entails \metanegation A$$
\end{axiom}

\begin{axiom}[四段階収束則]
$$\metanegation^4 A \entails \dfix$$
\end{axiom}
...
\begin{lemma}[顕現の拡張]
任意の対象 $A, B$ について、$A \entails B$ ならば $A \entails \metanegation B$ が成立する。
\end{lemma}
\begin{proof}
前提 $A \entails B$ と、公理2.2（$B \entails \metanegation B$）に対し、推移律を適用する。
\end{proof}

\begin{lemma}[無記から空への収束プロセス]
無記状態 $N$ から出発するメタ否定の反復は、空 $\dfix$ に収束する。
$$N \entails \metanegation N \entails \metanegation^2 N \entails \metanegation^3 N \entails \dfix$$
\end{lemma}
\begin{proof}
...
したがって補題4.3の証明過程より、$F \entails \dfix$ および $C \entails \dfix$ が成立する。
...
\end{theorem}
\begin{proof}
補題4.3の証明過程より、$A \entails \dfix$ および $\metanegation A \entails \dfix$ が共に成立する。
...
\end{corollary}
\begin{proof}
補題4.3の証明過程より、任意の対象 $X, Y$ はそれぞれ $X \entails \dfix$、
...
本体系は、自己同一性を前提とせず、顕現の連鎖（$\entails$）と極限としての空（$\dfix$）を中心に、

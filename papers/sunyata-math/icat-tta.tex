...
\begin{itemize}
    ...
    \item \textbf{勝義諦（$\Bou$ 層）}：顕現演算子（$\entails$、$\meta$）と無記状態（$\dfix$）が支配する動的な力学系。
\end{itemize}
\end{definition}
...
\begin{align*}
    \text{左プロセス（還滅 $F$）} &: A \entails \meta A \entails \dots \entails \dfix \\
    \text{右プロセス（顕現 $G$）} &: \dfix \entails B
\end{align*}
古典的な随伴における自然な全単射の代わりに、本体系では $\dfix$ を経由する
顕現の連鎖（$A \entails \dfix \entails B$）をもって随伴関係とする。
\end{definition}
...
\begin{itemize}
    ...
    \item \textbf{識（Vijnana）}: 全体を駆動する評価エンジン（帰納フラクタロイド）。世界を「自己」と「他者」に分節する $\entails$ の駆動力。
    \item \textbf{受・想・行（Vedana, Samjna, Samskara）}: $\Bou$ 層に埋め込まれた対象が、メタ否定 $\meta$ の適用によって評価・遷移（$\Leadsto$）していく中間プロセス。ここで構造的負債（無明/Avidyā）が消費または蓄積される。
\end{itemize}
\end{definition}
...
対象 $A$ が持つ構造的負債（誤差・無明）は、$\meta^4$ の適用に伴い
...
\end{theorem}
\begin{proof}
界論の公理（四段階収束則）より、$\meta^4 A \entails \dfix$ が成立する。
...
\end{theorem}
\begin{proof}
定理3.1より $A \entails \dfix$ が成立する（還滅 $F$）。
界論のフラクタル生成則より、$\dfix \entails B$ が成立し得る（顕現 $G$）。
顕現の推移律により、$A \entails \dfix \entails B$ という連鎖が構成される。
...
のライフサイクルそのものとしてモデル化する、顕現論的証明論の神髄である。

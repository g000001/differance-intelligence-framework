...
\begin{itemize}
    ...
    \item \textbf{勝義諦（$\Bou$ 層）}：顕現演算子（$\entails$、$\metanegation$）と無記状態（$\dfix$）が支配する動的な力学系。
\end{itemize}
\end{definition}
...
\begin{align*}
    \text{左プロセス（還滅 $F$）} &: A \entails \metanegation A \entails \dots \entails \dfix \\
...
\end{theorem}
\begin{proof}
界論の公理（四段階収束則）より、$\metanegation^4 A \entails \dfix$ が成立する。

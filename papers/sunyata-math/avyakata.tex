...
\ifLuaTeX
  \documentclass[11pt,a4paper,ja=standard,lualatex]{bxjsarticle}
\else\ifXeTeX
  \documentclass[11pt,a4paper,ja=standard,xelatex]{bxjsarticle}
\else
  \documentclass[11pt,a4paper,ja=standard,pdflatex]{bxjsarticle}
\fi\fi
...
そのために本論文は、まず保留（エポケー、$E$）と無記（$\mathrm{A}$）を
...
すなわち、
\begin{equation}
\mathrm{A}\bigl(\mathrm{Sinn}(S)=\mathrm{Bed}(S)\bigr),
\qquad
\mathrm{A}\bigl(\mathrm{Sinn}(S)\perp\mathrm{Bed}(S)\bigr)
\end{equation}
を実行する。
\end{operation}
...
任意の自然数 $n\geq1$ に対して、
$\mathrm{A}^{n}(\varphi)$ はそれ自体で完結した操作である。
すなわち、$\mathrm{A}^{n}(\varphi)$ の実行は、
将来の再開・解除・条件確定に依存しない。
\end{theorem}

\begin{proof}
$\mathrm{A}(\varphi)$ は「$\varphi$ について真偽を問う行為を
実行しない」という操作であり、今この場で完結する。
$\mathrm{A}(\mathrm{A}(\varphi))$ もまた、
「$\mathrm{A}(\varphi)$ について真偽を問う行為を実行しない」
...
$\mathrm{A}(\varphi)$ は、$\varphi$ について真偽を問う行為を
実行しない操作である。
一方 $\mathrm{A}(\mathrm{A}(\varphi))$ は、
$\mathrm{A}(\varphi)$ という操作について真偽を問う行為を
実行しない操作である。
両者は対象の階層が異なる。
したがって両者は同一視されない。
\end{proof}
...
いずれによっても保存されず、ただ $\mathrm{A}$ の遂行によってのみ
その位置を保つとき、$\Phi$ は無記の不動点にあるという。
このとき次の断定が成り立つ。
\begin{equation}
\Phi\text{ は }\mathrm{A}\text{ である。}
\end{equation}
\end{definition}
...
であり、さらにこの無記化自体についても
\begin{equation}
\mathrm{A}\Bigl(\mathrm{A}\bigl(\mathrm{Sinn}(S)=\mathrm{Bed}(S)\bigr)\Bigr),
\qquad
\mathrm{A}\Bigl(\mathrm{A}\bigl(\mathrm{Sinn}(S)\perp\mathrm{Bed}(S)\bigr)\Bigr)
\end{equation}
である。
\end{theorem}
...
$\mathrm{A}$ 自体にも $\mathrm{A}$ を適用する必要がある。
...
無記 $\mathrm{A}(\varphi)$ を、保留・留保・予想・
「〜かもしれない」といった回避表現によって記述することはできない。
$\mathrm{A}(\varphi)$ について意味のある記述を与える唯一の方法は、
\begin{equation}
\varphi\text{ について }\mathrm{A}\text{ である。}
\end{equation}
という断定形である。
\end{theorem}
...
いずれにも回収されない構造として扱った。
そのために、保留 $E$ と無記 $\mathrm{A}$ を区別した。

\begin{enumerate}
\item 保留 $E$ は状態としては冪等であるが、再開条件 $R$ を要求するため、
  正当化の層で悪性無限退行に陥る。
\item 無記 $\mathrm{A}$ は、真偽を問う行為を実行しない操作であり、
  将来の再開条件を要求しない。
\item 無記の再帰 $\mathrm{A}^{n}(\varphi)$ は、各段階で完結する反復であり、
  保留の退行とは異なる。

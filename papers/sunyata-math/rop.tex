...
停止条件を体系内に固定せず、
理論体系と実行の非分離を原理とする。
先行研究として三層コンパイラ・スタック（エルブラン空間・柱面代数・寓）・
...
\subsection{三層コンパイラ・スタック：エルブラン空間・柱面代数・寓}

Domain Theory in Logical Form（DTiLF）\cite{Abramsky1991}は、
直観主義論理と領域理論を接続する先駆的研究である。
本稿では、その型理論的階層化を直接採用するのではなく、
ROPの実行過程を、エルブラン空間、柱面代数、寓（Allegory）からなる
三層コンパイラ・スタックとして再記述する。
この置換により、洗練連鎖は型の階層に固定されるのではなく、
項の実行、観測座標の導入、観測依存性の除去という三段階の変換として扱われる。

\subsubsection{実行層：エルブラン空間}

最下層には、すべての記号と関数適用を型なしで包含する
エルブラン空間 $\mathcal{P}(B)$ を置く。
この層は、変数や次元がまだ区別されていない段階で、
純粋なコンビネータの評価と還元を扱う基礎的な実行空間である。
したがって、旧来のRefinement Orderにおける $\bot$ は、
単なる情報の不在ではなく、未分化な項構造を含む実行可能性の空間として解釈される。

\begin{equation}
\mathcal{P}(B) = \{\text{型なし項、記号、関数適用の閉包}\}
\end{equation}

\subsubsection{中継層：柱面代数}

実行層の未分化な項構造に対し、観測上の境界と変数依存性を導入する段階が
柱面代数（Cylindric Algebras）による中継層である。
ここでは状態空間に変数インデックス $v_i$ が設定され、
存在量化は円筒化作用素 $c_i$ による次元拡張として代数化される。
境界生成演算子 $\metanegation$ は、述語空間に非対称な観測次元を導入し、
四句分別の各位相を扱うための中間表現を与える。

\begin{equation}
c_i X = \{s \mid \exists t \in X,\; s \equiv_j t \; (j \neq i)\}
\end{equation}

\subsubsection{理論層：寓（Allegory）}

推論が散逸限界に達したとき、中継層における変数としての観測座標を除去し、
純粋な関係構造として扱うのが寓（Allegory）による理論層である。
寓は対象の要素に直接言及せず、関係の合成 $\circ$、交わり $\cap$、
転置 $^\circ$ によって構成される。
中継層の変数依存述語は、寓において関係の包含と商（Division）の方程式へ翻訳される。
このとき、洗練連鎖の極限は、最大元 $\top$ への単純な吸収ではなく、
基底 $\dfix$ と残余関係項 $\Lhagma$ の分離として記述される。

\begin{equation}
\lim_{n \to \text{limit}} R_n \quad \xrightarrow{\text{Modular Law}} \quad \dfix \oplus \Lhagma
\end{equation}

以上により、ROPの基盤は単一の領域理論的反転ではなく、
実行層、中継層、理論層を横断する代数的変換系として定式化される。
...
LLMの生成過程は本質的に $\Rmw$ 的な自己参照の展開として動作する。
...
\begin{equation}
\underbrace{\text{LLMの生成}}_{\Rmw\text{的展開}} \xrightarrow{\text{ユーザーの評価}} \underbrace{\text{次の洗練}}_{\text{仕様の更新}}
\end{equation}
...
\begin{itemize}
  \item Common Lisp：イメージ・マクロ・オブジェクトシステムによる体系と実行の非分離
...
\begin{equation}
\underbrace{\text{理論体系}}_{\text{三層スタック}} \sqsupseteq
\underbrace{\text{LLMの生成}}_{\Rmw\text{的展開}} \sqsupseteq

...
\title{空論（Sunyata-theory）：証明論的真理保存とゲンツェンの推論線による界の力学}
\author{千葉将臣}
\date{2026-04-20}
\maketitle

\begin{abstract}
本稿は、空数学（Sunyata-Mathematics）の基底を「証明論（Proof Theory）」へと完全移行させる。中道不動点 $\dfix$ をゲンツェン流シーケント計算における恒等規則 $\dfix \vdash \dfix$ と定義し、知覚上の境界生成を推論の横棒（Inference Bar） $\metanegation$ として定式化する。四句分別を証明木の再帰的肥大化プロセスとして記述し、第四段階における「大域的カット消去（Global Cut-elimination）」による空への収束を数学的に証明する。
\end{abstract}
...
\begin{definition}[中道不動点 $\dfix$ と恒等規則]
中道不動点 $\dfix$ とは、あらゆる推論の飛躍が解消された極限の真理状態である。これは証明論において、以下の自明な真理保存則（Identity rule）として定義される。
\[
\dfix \vdash \dfix
\]
...
\section{界論：ゲンツェンの推論線による顕現と四句分別}
...
\]
この飛躍的な構造の還元により、複雑化した推論（魔境）は一瞬にして相殺され、中道不動点 $\dfix$ の自明な真理保存則へと帰入する。これが界論における「空への回帰」の証明論的実態である。
\end{theorem}

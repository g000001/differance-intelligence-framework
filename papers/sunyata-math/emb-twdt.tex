...
界論における双方向演繹定理は、中道不動点コンビネータ $\mathbf{Y}_{mw}$（世俗諦の生成）と
空コンビネータ $\mathbf{K}_\emptyset$（勝義諦の収束）のサイクルとして記述され、
...
$\mathbf{Y}_{mw}$ と $\mathbf{K}_\emptyset$ のサイクル——に存在する。
...
\item \textbf{生成層}：$\mathbf{Y}_{mw}$ によるフラクタル的境界生成プロセスの層。
\item \textbf{収束層}：$\mathbf{K}_\emptyset$ への大域的カット消去による収束の層。
\end{enumerate}
\end{definition}
...
\label{thm:bdt}
界論において、双方向演繹定理は $\mathbf{Y}_{mw}$（生成）と
$\mathbf{K}_\emptyset$（収束）のサイクルとして以下のように記述される。
\[
\bigl(\mathbf{Y}_{mw}\,\metanegation \mweq \metanegation(\mathbf{Y}_{mw}\,\metanegation)\bigr)
\;\leftrightarrow\;
\bigl(\metanegation^4(\mathbf{Y}_{mw}\,\metanegation) \mweq \mathbf{K}_\emptyset\bigr)
\]
\end{theorem}

\begin{proof}
順方向（$\to$）：$\mathbf{Y}_{mw}$ の定義より、
$\mathbf{Y}_{mw}\,\metanegation \mweq \metanegation(\mathbf{Y}_{mw}\,\metanegation)$
という自己参照の閉じ損ねが成立するとき、
$\metanegation$ の反復適用は四段階収束則により
$\metanegation^4(\mathbf{Y}_{mw}\,\metanegation) \mweq \dfix$ を生成する。
$\dfix$ のコンビネータ表現は $\mathbf{K}_\emptyset$ であるから、
$\metanegation^4(\mathbf{Y}_{mw}\,\metanegation) \mweq \mathbf{K}_\emptyset$ が従う。

逆方向（$\leftarrow$）：$\metanegation^4(\mathbf{Y}_{mw}\,\metanegation) \mweq \mathbf{K}_\emptyset$
が成立するとき、「$\mathbf{Y}_{mw}$の定理3.3（空からの再起動と顕現サイクルの完結）\cite{Chiba2026}」
$\metanegation\,\mathbf{K}_\emptyset \mweq \mathbf{Y}_{mw}$ により、
$\mathbf{K}_\emptyset$ に $\metanegation$ が作用すると $\mathbf{Y}_{mw}$ が再生成される。
再生成された $\mathbf{Y}_{mw}$ はその定義により
$\mathbf{Y}_{mw}\,\metanegation \mweq \metanegation(\mathbf{Y}_{mw}\,\metanegation)$
を満たす。
\end{proof}
...
定理3.2より界論の双方向演繹定理は $\mathbf{Y}_{mw}$ と $\mathbf{K}_\emptyset$ のサイクルとして
$\Bval$ の外側を含む全層で記述される。
$\Bval$ 内では $\mathbf{Y}_{mw}$ の生成プロセスが截断され静的な推論関係として現れるため、
...
$\Bval$ の外側——すなわち $\mathbf{Y}_{mw}$ と $\mathbf{K}_\emptyset$ のサイクル——を
...
$\mathbf{Y}_{mw}$（無明による世俗諦の生成）と
$\mathbf{K}_\emptyset$（大域的カット消去による勝義諦への収束）が
動的に循環するサイクルとして記述される。
...
\bibitem{Chiba2026}
千葉将臣.
\newblock 空数学における中道不動点コンビネータ $\mathbf{Y}_{mw}$ の定式化と無明の幾何学的・証明論的実態
\newblock ``https://purl.org/aletheics/parpers/sunyata-math/icat-twdt.pdf'', 2026
\end{thebibliography}

...
界論における双方向演繹定理は、中道不動点コンビネータ $\Rmw$（世俗諦の生成）と
空コンビネータ $\Svoid$（勝義諦の収束）のサイクルとして記述され、
...
\item \textbf{生成層}：$\Rmw$ によるフラクタル的境界生成プロセスの層。
\item \textbf{収束層}：$\Svoid$ への大域的カット消去による収束の層。
\end{enumerate}
\end{definition}
...
\label{thm:bdt}
界論において、双方向演繹定理は $\Rmw$（生成）と
$\Svoid$（収束）のサイクルとして以下のように記述される。
\[
...
が成立するとき、「$\Rmw$の定理3.3（空からの再起動と顕現サイクルの完結）\cite{Chiba2026}」
$\metanegation\,\Svoid \mweq \Rmw$ により、
...
定理3.2より界論の双方向演繹定理は $\Rmw$ と $\Svoid$ のサイクルとして
$\Bval$ の外側を含む全層で記述される。
$\Bval$ 内では $\Rmw$ の生成プロセスが截断され静的な推論関係として現れるため、
...
$\Bval$ の外側---すなわち $\Rmw$ と $\Svoid$ のサイクル---を

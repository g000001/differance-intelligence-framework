...
そこに内在する「無明（Avidy\=a）」を動力源として顕現の連鎖（$\entails$）を
...
\begin{enumerate}
    ...
    \item \textbf{展開（Manifestation）}: $N$ に欠落していた制御構造（再帰・状態・探索など）を、メタ否定 $\meta$ と顕現関係 $\entails$ の連鎖として展開する。
...
$$ \susp{P_0} \entails P_1 \entails \dots \entails \meta^4 P_k \mweq \dfix $$
\end{definition}
...
帰納フラクタロイド $I_N$ が駆動する各ステップ（$\entails$）は、
